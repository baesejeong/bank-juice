    ...
    <div class="d-flex flex-column align-items-center mt-2 mb-5">
      ...
      <table class="table">
        ...
        <tbody class="text-center">
          ...
          @click="goDetail(deposit.deposit)"
          class="product"
          >
          <th scope="row">{{ index + 1 }}</th>
          <td>{{ deposit.intr_rate_6 ? deposit.intr_rate_6 : '-' }}</td>
          <td>{{ deposit.intr_rate_12 ? deposit.intr_rate_12 : '-' }}</td>
          <td>
            <vue-number-format
              class="number-form text-center"
              :value="deposit.interest_before_tax" 
              :options="{ 
                thousand: ',',
                decimal: '.',
                prefix: '',
          ...
          <td>
            <vue-number-format
              class="number-form text-center"
              :value="deposit.interest_after_tax" 
              :options="{ 
                thousand: ',',
                decimal: '.',
                prefix: '',
          ...
          <td>{{ deposit.intr_rate_24 ? deposit.intr_rate_24 : '-' }}</td>
          <td>{{ deposit.intr_rate_36 ? deposit.intr_rate_36 : '-' }}</td>
...
const searchProduct = function () {
  axios({
    method: 'get',
    url: `${store.API_URL}/info/deposit/`,
    params: {
      tariff: tariff.value,
