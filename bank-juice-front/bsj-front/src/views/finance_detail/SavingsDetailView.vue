    ...
    <div v-if="savings" class="mt-5 detail-div ">
      ...
      <div class="d-flex flex-row justify-content-between">
                ...
                <div class="card my-2 p-4 d-flex flex-column">
                  <div class="d-flex flex-row justify-content-between">
                    <div class="title">
                      ...
                      <span class="my-4">은행 : {{ savings.kor_co_nm }}</span>
                      <p class="mt-3 mb-3">가입 방법 : {{ savings.join_way }}</p>
                    </div>
        ...
        <div class="side-div mt-3 mb-6">
          <KakaoMap :bank-name="savings.kor_co_nm" v-if="savings" class="kakao-map" style="width: 500px;"/>
          <div class="mt-2">
            <div>
...
const recommended_products_sliced = computed(()=>{
  ...
  for (let i = 0 ; i < recommended_products.value.length ; i ++) {
    // console.log(recommended_products[i])
    if (recommended_products[i]=== savings.value.fin_prdt_nm ) {
      // console.log(recommended_products[i])
      continue
    }    
...
const goDetail = function (savingsPk) {
  axios({
    method: 'get',
    url: `${store.API_URL}/info/savings/${savingsPk}/`,
  })
    .then((res) => {
    ...
    // 로컬 스토리지에 최근 본 상품 저장
    .then((res) => {
      const find_savings = store.recent_savings.find((depo) => depo.id == savings.value.id)
      if (store.recent_savings.length === 5) {
        if (!find_savings) {
          store.recent_savings.shift()
          store.recent_savings.push(savings.value)
        }
      } else {
        if (!find_savings) {
          store.recent_savings.push(savings.value)
    ...
    .then((res)=>{
      ...
      axios({
        method : 'get',
        url : `${store.API_URL}/info/bank/${savings.value.bank}/`
      })
        .then((res)=>{
    ...
    .then((res)=>{
      axios({
        method:'get',
        url:`${store.API_URL}/info/savings_save/${savingsPk}/`
      })
      .then((res)=>{
...
const productSave = function(event) {
  axios({
    method:'post',
    url: `${store.API_URL}/info/savings_save/${route.params.pk}/`,
    headers: {
          Authorization: `Token ${store.token}`
        }
  })
...
const reload = function () {
  axios({
    method: 'get',
    url: `${store.API_URL}/info/savings/${route.params.pk}/`,
  })
    .then((res) => {
      savings.value = res.data
      console.log(savings.value)
...
onMounted(() => {
  reload()
  console.log(store.recent_savings == null)
  if (!store.recent_savings == null) {
    const find_savings = store.recent_savings.find((depo) => depo.id === savings.value.id)
    if (store.recent_savings.length === 5) {
      if (!find_savings) {
        store.recent_savings.shift()
    ...
    store.recent_savings = [savings.value]
  }
})
