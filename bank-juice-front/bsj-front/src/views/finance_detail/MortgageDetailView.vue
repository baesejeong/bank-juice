    ...
    <div class="mt-5 detail-div ">
      ...
      <div class="d-flex flex-row justify-content-between">
        <div class="card-div d-flex flex-column align-items-center">
          <div v-if="mortgage" class="mt-1 mb-6 recommend-div">
            <div>
              <div>
                <div class="card my-2 p-4 d-flex flex-column">
                  <div class="d-flex flex-row justify-content-between">
                    <div class="title">
                      ...
                      <span class="my-4">은행 : {{ mortgage.kor_co_nm }}</span>
                      <p class="mt-3 mb-3">가입 방법 : {{ mortgage.join_way }}</p>
                    </div>
        ...
        <div class="side-div mt-3 mb-6">
          <KakaoMap :bank-name="mortgage.kor_co_nm" v-if="mortgage" class="kakao-map"/>
          <div class="mt-2">
            <div>
...
const recommended_products_sliced = computed(()=>{
  ...
  for (let i = 0 ; i < recommended_products.value.length ; i ++) {
    if (recommended_products[i]=== mortgage.value.fin_prdt_nm ) {
      continue
    }    
...
const goDetail = function (mortgagePk) {
  axios({
    method: 'get',
    url: `${store.API_URL}/info/mortgage/${mortgagePk}/`,
  })
  .then((res) => {
      console.log(res.data)
      mortgage.value = res.data
      opts.value = res.data.options
      option1.value = res.data.options[0]
    })
    // 로컬 스토리지에 최근 본 상품 저장
    .then((res) => {
      if (!store.recent_mortgage == null) {
        const find_mortgage = store.recent_mortgage.find((depo) => depo.id == mortgage.value.id)
        if (store.recent_mortgage.length === 5) {
          if (!find_mortgage) {
            store.recent_mortgage.shift()
            store.recent_mortgage.push(mortgage.value)
          }
        } else {
          if (!find_mortgage) {
            store.recent_mortgage.push(mortgage.value)
        ...
        store.recent_mortgage = [mortgage.value]
      }
    })
    //은행 정보
    .then((res)=>{
      ...
      axios({
        method : 'get',
        url : `${store.API_URL}/info/bank/${mortgage.value.bank}/`
      })
        .then((res)=>{
    ...
    .then((res) => {
      axios({
        method:'get',
        url:`${store.API_URL}/info/mortgage_save/${mortgagePk}/`
      })
      .then((res)=>{
...
const productSave = function(event) {
  axios({
    method:'post',
    url: `${store.API_URL}/info/mortgage_save/${route.params.pk}/`,
    headers: {
          Authorization: `Token ${store.token}`
        }
  })
...
const reload = function () {
  axios({
    method: 'get',
    url: `${store.API_URL}/info/mortgage/${route.params.pk}/`,
  })
  .then((res) => {
      console.log(res.data)
      mortgage.value = res.data
